--  SPDX-FileCopyrightText: 2021 Rolf Ebert <rolf.ebert.gcc@gmx.de>
--
...
with Ada.Streams;
with ESP32.UART_Types; use ESP32.UART_Types;
with ESP32.GPIO;       use ESP32.GPIO;
...
package ESP32.UART is
   ...
   subtype Uart_Data_Index is Ada.Streams.Stream_Element_Offset;
   subtype Uart_Data is Ada.Streams.Stream_Element_Array;

   -- Valid UART port number
   Port_Num_Max : constant := 2;
   type Port_Num is new Integer range 0 .. Port_Num_Max;


   Fifo_Len    : constant := 128;       -- UART hw FIFO length
   Bitrate_Max : constant := 5_000_000; -- max bit rate supported by UART

   -- UART interrupt configuration parameters for uart_intr_config function
   type Interrupt_Config is record
      Interrupt_Enable_Mask    : Unsigned_32;  --  UART interrupt enable mask, choose from UART_XXXX_INT_ENA_M under UART_INT_ENA_REG(i), connect with bit-or operator
      Rx_Timeout_Thresh        : Unsigned_8;   -- UART timeout interrupt threshold (unit: time of sending one byte)
      Txfifo_Empty_Intr_Thresh : Unsigned_8;   -- UART TX empty interrupt threshold.
      Rxfifo_Full_Thresh       : Unsigned_8;   -- UART RX full interrupt threshold.
   end record;
   ...
   type Event_Type_T is (
                         DATA,              -- UART data event
                         BREAK,             -- UART break event
                         BUFFER_FULL,       -- UART RX buffer full event
                         FIFO_OVF,          -- UART FIFO overflow event
                         FRAME_ERR,         -- UART RX frame error event
                         PARITY_ERR,        -- UART RX parity event
                         DATA_BREAK,        -- UART TX data and break event
                         Pattern_Det,       -- UART pattern detected
                         Event_Max);        -- UART event max index



   -- Event structure used in UART event queue
   type Event_Info is record
      Kind         : event_type_t; -- UART event type
      Size         : Natural;      -- UART data size for UART_DATA event
      Timeout_Flag : Boolean;      -- UART data read timeout flag for UART_DATA event (no
                                   -- new data received during configured RX TOUT).  If
                                   -- the event is caused by FIFO-full interrupt, then
                                   -- there will be no event with the timeout flag before
                                   -- the next byte coming.
   end record;
   ...
   --  *        If set to zero, driver will not use TX buffer, TX function will block task until
   --  *        all data have been sent out.
   ...
   procedure Driver_Install (Port : Port_Num;
                             Rx_Buffer_Size : Natural;
                             Tx_Buffer_Size : Natural;
                             Queue_Size : Natural;
   ...
   procedure Driver_Delete (Port : Port_Num);


   --  Checks whether the driver is installed or not
   function Is_Driver_Installed (Port : Port_Num) return Boolean;


   --  Set UART data bits.
   procedure Set_Word_Length (Port : Port_Num; Word_Bits : Nr_Of_Data_Bits);


   --  Get the UART data bit configuration.
   function Get_Word_Length (Port : Port_Num) return Nr_Of_Data_Bits;


   --  Set UART stop bits.
   procedure Set_Stop_Bits (Port : Port_Num; Bits : Nr_Of_Stop_Bits);


   --  Get the UART stop bit configuration.
   function Get_Stop_Bits (Port : Port_Num) return Nr_Of_Stop_Bits;


   --  Set UART parity mode.
   procedure Set_Parity (Port : Port_Num; Parity : Parity_Mode);
...
--  esp_err_t uart_isr_free(uart_port_t uart_num);
   ...
   --   *
   --   * @note Instead of GPIO number the constant 'UART_PIN_NO_CHANGE' may be provided
   ...
   --   *
   procedure Set_Pin (Port       : Port_Num;
                      Tx_IO_Num  : GPIO_Pad := Pin_No_Change;
...
--  esp_err_t uart_intr_config(uart_port_t uart_num, const uart_intr_config_t *intr_conf);
   ...
   --  procedure Wait_Tx_Done (Port : Port_Num; Ticks_To_Wait : Natural; Result : out Timeout_Or_Empty);
...
--  int uart_write_bytes_with_break(uart_port_t uart_num, const void* src, size_t size, int brk_len);
   ...
   --   */
   procedure Read_Bytes (Port              :     Port_Num;
                         Data              : out Uart_Data;
                         Length            : out Uart_Data_Index;
                         Milliseconds_Wait :     Natural);
